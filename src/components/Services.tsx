  ...
  return <section className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">Each venue is different and has a different combination of services. Our specialized mobile applications are crafted to enhance the specific experiences that make your establishment exceptional.</p>
        </div>

        {/* Services Grid */}
        <div className="space-y-20">
          {services.map((service, index) => <div key={index} className={`grid lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
              {/* Content */}
              ...
              <div className={`${index % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''}`}>
                <div className="relative rounded-2xl overflow-hidden shadow-primary">
                  <img src={service.image} alt={service.title} className="w-full h-80 object-cover transition-smooth hover:scale-105" />
